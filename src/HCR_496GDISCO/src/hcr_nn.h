...
#ifndef __AI_HCR_NN_H__
#define __AI_HCR_NN_H__
#pragma once

#include "ai_platform.h"

#define AI_HCR_NN_MODEL_NAME          "hcr_nn"

#define AI_HCR_NN_IN_NUM       (1)
#define AI_HCR_NN_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 28, 28, 1, 1, NULL), \
}
#define AI_HCR_NN_IN_SIZE { \
  (28 * 28 * 1), \
}
#define AI_HCR_NN_IN_1_SIZE  (28 * 28 * 1)
#define AI_HCR_NN_IN_1_SIZE_BYTES  ((28 * 28 * 1) * 4)


#define AI_HCR_NN_OUT_NUM      (1)
#define AI_HCR_NN_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 36, 1, NULL), \
}
#define AI_HCR_NN_OUT_SIZE { \
  (1 * 1 * 36), \
}
#define AI_HCR_NN_OUT_1_SIZE  (1 * 1 * 36)
#define AI_HCR_NN_OUT_1_SIZE_BYTES  ((1 * 1 * 36) * 4)
 ...
 * @ref ai_hcr_nn_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_HCR_NN_*
 * defines into file @ref hcr_nn.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_hcr_nn_forward(
  ai_handle network, const ai_buffer* input);


AI_API_DECLARE_END

#endif /*__AI_HCR_NN_H__*/
